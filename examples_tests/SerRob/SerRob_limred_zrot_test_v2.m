...
%                                                                                             'Phit_tol', 1e-12, 'Phir_tol', 1e-12));
  ...
  % Initialisierung
  h5_ep = NaN(1+n_max, amount_optcrit, size(XE,1)); % h5 kann auch ausgelesen werden, wenn wn(5) nicht aktiv ist
  h6_ep = h5_ep;
  phiz_ep = h5_ep;
...
%     q0_ep = RS.qref + [0;0;0;0;0;-80]*pi/180; % spez. Wert festlegen
    ...
    % Structs definieren
    s_ep_wn = zeros(6,amount_optcrit);  % 3T2R mit wn(:) = 0
    s_ep_wn(5:6,2) = [0;1];  % limred mit hyperbolischem Ansatz
    s_ep_wn(5:6,3) = [1;1];  % limred mit quadratischem und hyperbolischem Ansatz
    opt1 = s_ep_wn(5:6,2);   % Sicherung für späteren Plot
    opt2 = s_ep_wn(5:6,3);   % Sicherung für späteren Plot
    s_ep = struct( ...
    ...
    for k = 1:amount_optcrit
      ...
      h5_ep(:,k,i)   = Stats_IK_ep.xlimred_quad(:);
      h6_ep(:,k,i)   = Stats_IK_ep.xlimred_hyp(:);
...
%       end
      ...
      for kk = 1:amount_optcrit
        ...
        if s_ep_wn(6,kk) == 1
          ...
          for nn = 1:index_phiz(end)
            if isinf(h6_ep(nn,kk,pkt))
              plot(nn, 0,'*g', 'DisplayName','h(6) = inf');
            end
          end
          hold on;
          grid on;
        ...
        if s_ep_wn(5,kk) == 1 || s_ep_wn(6,kk) == 1
  ...
  h7_k = NaN(size(T,1),size(Namen_Methoden,2));
  h8_k = h7_k;
  h9_k = h7_k;
  ...
  for k = 1:length(Namen_Methoden)
    % Default-Struct
    s_traj_wn = zeros(15,1); % wn(:)=0 -> keine Opt.Krit
    ...
    switch k
      ...
      case 2
        % wn(13:14)=1: hyp. für qD und qDD
        s_traj.wn(13:14) = 1;
        name_method = sprintf('3T2R-IK mit wn(13:14)=1');
        name_method_save2 = name_method;
      case 3
        % wn(15)=1: quadr. für qD
        s_traj.wn(15) = 1;
        name_method = sprintf('3T2R-IK mit wn(15)=1');
        name_method_save3 = name_method;
      case 4
        % wn(13:15)=1: Einfluss von fehlendem wn(11:12) testen
        s_traj.wn(13:15) = 1;
        name_method = sprintf('3T2R-IK mit wn(13:15)=1');
        name_method_save4 = name_method;
      case 5  % vollständiges Optimierungskriterium immer ans Ende stellen
        % wn(11:15)=1: Alle Optimierungskriterien aktivieren
        s_traj.wn(11:15) = 1;
        name_method = sprintf('3T2R-IK mit wn(11:15)=1');
        name_method_save5 = name_method;
    end
    wn_limred_save(:,k) = s_traj.wn(11:15);
    ...
    % Auswertung für den Plot: h(7) bis h(9)
    h7_k(:,k) = Stats_Traj_k.h(:,8);
  ...
  if traj_plot_needed
    ...
    for kk = 1:size(Namen_Methoden,2)
      ...
      if wn_limred_save(1,kk) == 1 || wn_limred_save(2,kk) == 1
        ...
        plot(T, h7_k(:,kk), 'm', 'LineWidth',1);
        axe = gca;
        axe.XLim = [0 T(end)];
        hold on;
        if ylable_first_plot_incolumn(2) == true
          ylabel('Optim.krit h(7)');
          ylable_first_plot_incolumn(2) = false;
        end
        xlabel('Zeit in s'); grid on;
      end
      ...
      % Subplots Vierte Zeile: Verlauf von h(9) -------------------------------------
