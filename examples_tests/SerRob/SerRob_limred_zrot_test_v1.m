...
calc_traj = false;  % IK für Trajektorie berechnen
calc_ep = true;
...
% RS.anim( Q(1:50:end,:), [], s_anim, s_plot);
  ...
  if calc_ep
    ...
    h5_save_limred2    = h5_save_nolimred;
    h6_save_nolimred   = h5_save_nolimred;
    h6_save_limred     = h5_save_nolimred;
    h6_save_limred2    = h5_save_nolimred;
    phiz_save_nolimred = h6_save_nolimred;
    phiz_save_limred   = h6_save_nolimred;
    phiz_save_limred2  = h6_save_nolimred; 
    iter_limred        = NaN(size(XE,1),1);
    iter_limred2       = iter_limred;
    iter_nolimred      = iter_limred;
    Q_save_nolimred    = NaN(1+n_max,6,size(XE,1));
    Q_save_limred      = Q_save_nolimred;
    Q_save_limred2     = Q_save_nolimred;
    wn5_active_nolimred = zeros(1+n_max,size(XE,1));
    wn5_active_limred = wn5_active_nolimred;
    wn5_active_limred2= wn5_active_nolimred;
    angle_nolimred = NaN(size(XE,1),1);
    angle_limred = NaN(size(XE,1),1);
    angle_limred2= NaN(size(XE,1),1);
    
    q0_diff = -80; % in [deg]
    q0_ep = RS.qref + [0;0;0;0;0;q0_diff]*pi/180;
%     q0_rng = 50; % in [deg]
%     q0_ep = RS.qref-q0_rng*pi/180*(0.5-rand(RS.NQJ,1)); % Anfangswinkel q0_rng° variieren
...
%       RS.I_EE_Task = logical([1 1 1 1 1 1]);
%       [q_ziel, phi_ziel,~,Stats_ziel] = RS.invkin2(RS.x2tr(XE(i,:)'), q0_ep, struct('n_max', n_max, ...
...
% %       end
      ...
      s_ep_3T2R = struct( ...
        ...
        'scale_lim', 0, 'reci', true, 'wn', zeros(6,1), 'retry_limit', 0);
      s_ep_3T2R.xlim = [NaN(5,2); [-45 45]*pi/180]; % in [rad] übergeben
      
      s_ep_limred = s_ep_3T2R;
      s_ep_limred.wn(5:6) = [0;1];  % limred mit hyperbolischem Ansatz testen

      s_ep_limred2 = s_ep_3T2R;
      s_ep_limred2.wn(5:6) = [1;1]; % limred mit quadratischem und hyperbolischem Ansatz testen
      ...
      % Test-Berechnung ob wn(5) aktiv ist/sein sollte (phi<1e-3)
      ...
      % Test-Berechnung ob wn(5) aktiv ist/sein sollte
      for nn = 1:(Stats_limred2.iter+1)
        Stats_Phi_temp2 = Stats_limred2.PHI(nn,:);
        if all(abs(Stats_Phi_temp2(logical([1 1 1 0 1 1])))<1e-3)  % vierten Eintrag ignorieren
          wn5_active_limred2(nn,i) = 1;
        end
      end
      if Stats_limred2.iter == n_max
        warning('limred2: Maximale Iteration bei Punkt %d erreicht', i);
      end
      ...
      if any(abs(phi_IK2) > Phirt_tol)
        error('Inverse Kinematik MIT limred2 fehlerhaft bei Punkt %d', i);
      end
    ...
    for jj = 1:size(XE,1)
      ...
      index_phiz_limred2 = 1:(iter_limred2(jj)+1);
      index_phiz_nolimred = 1:(iter_nolimred(jj)+1);

      
      % Subplots Erste Spalte -------------------------------------
      subplot(5,18,1:6); hold on;
      ...
      axe = gca;
      axe.XLim = [index_phiz_nolimred(1) index_phiz_nolimred(end)];
      ylabel('phi_z in °'); grid on;
      title('OHNE limred: [0 0 0 0 0]');
      
%       % h(5)
%       subplot(5,18,19:24); hold on;
...
%       
%       % h(6)
%       subplot(5,18,37:42); hold on;
%       plot(index_phiz_nolimred, h6_save_nolimred(1:index_phiz_nolimred(end),jj), 'm', 'LineWidth', 1');
%       axe = gca;
%       axe.XLim = [index_phiz_nolimred(1) index_phiz_nolimred(end)];
%       ylabel('Optimierungskriterium h(6)'); grid on;
%       for kk = 1:index_phiz_nolimred(end)
%         if isinf(h6_save_nolimred(kk,jj))
%           plot(kk, 0,'*g');
%         end
%       end
%       hold on;
...
%       xlabel('Iteration lfd Nr');
      ...
      axe = gca;
      axe.XLim = [index_phiz_limred(1) index_phiz_limred(end)];
      grid on;
      title('MIT limred hyp: [0 0 0 0 1]');

%       % h(5)
%       subplot(5,18,25:30); hold on;
%       plot(index_phiz_limred, h5_save_limred(1:index_phiz_limred(end),jj), 'm', 'LineWidth',1, 'DisplayName','h(5) aus IK');
%       for kk = 1:index_phiz_limred(end)
%         if isinf(h5_save_limred(kk,jj))
%           plot(kk, 0,'*g', 'DisplayName','h(5) = inf');
%         end
%       end
%       hold on;
%       grid on;
...
%       axe.XLim = [index_phiz_limred(1) index_phiz_limred(end)];
      ...
      % h(6)
      subplot(5,18,43:48); hold on;
      leg1(1) = plot(index_phiz_limred, h6_save_limred(1:index_phiz_limred(end),jj), 'm', 'LineWidth',1, 'DisplayName','h(6) aus IK');
      hinf_used = false;  % Abfrage bzgl. Legendeneintrag
      for kk = 1:index_phiz_limred(end)
        if isinf(h6_save_limred(kk,jj))
          hinf_used = true;
          leg1(2) = plot(kk, 0,'*g', 'DisplayName','h(6) = inf');
        end
      end
      hold on;
      grid on;
      ...
      % Subplots Dritte Spalte -------------------------------------
      subplot(5,18,13:18); hold on;
      plot(index_phiz_limred2, phiz_save_limred2(1:index_phiz_limred2(end),jj)*180/pi, 'm', 'LineWidth',1);
      xlim_vek2 = repmat([s_ep_limred2.xlim(6,1) s_ep_limred2.xlim(6,2)]*180/pi,index_phiz_limred2(end),1);
      plot(index_phiz_limred2, xlim_vek2(:,1), 'r--', 'LineWidth',1);
      plot(index_phiz_limred2, xlim_vek2(:,2), 'r--', 'LineWidth',1);
      plot(index_phiz_limred2, xlim_vek2(:,1)*0.8, 'b--', 'LineWidth',1);
      plot(index_phiz_limred2, xlim_vek2(:,2)*0.8, 'b--', 'LineWidth',1);
      axe = gca;
      axe.XLim = [index_phiz_limred2(1) index_phiz_limred2(end)];
      grid on;
      title('MIT limred quad+hyp: [0 0 0 1 1]');
      
      % h(5)
      subplot(5,18,31:36); hold on;
      plot(index_phiz_limred2, h5_save_limred2(1:index_phiz_limred2(end),jj), 'm', 'LineWidth',1);
      for kk = 1:index_phiz_limred2(end)
        if isinf(h5_save_limred2(kk,jj))
          plot(kk, 0,'*g', 'DisplayName','h(5) = inf');
        end
      end
      hold on;
      ...
      % h(6)
      subplot(5,18,49:54); hold on;
      plot(index_phiz_limred2, h6_save_limred2(1:index_phiz_limred2(end),jj), 'm', 'LineWidth',1);
      for kk = 1:index_phiz_limred2(end)
        if isinf(h6_save_limred2(kk,jj))
          plot(kk, 0,'*g', 'DisplayName','h(6) = inf');
        end
      end
      hold on;
      grid on;
      ...
      % Zeile 4
      % Subplots Erste Spalte -------------------------------------
%       subplot(5,18,55:60); hold on;
%       for kk = 1:index_phiz_nolimred(end)
%         if wn5_active_nolimred(kk,jj) == 1
%           plot(kk, 1,'om', 'DisplayName','limred aktiv');
%         elseif wn5_active_nolimred(kk,jj) == 0
%           plot(kk, 0,'or', 'DisplayName','limred inaktiv');
%         end
%       end
%       hold on;
%       axe = gca;
%       axe.XLim = [index_phiz_nolimred(1) index_phiz_nolimred(end)];
%       axe.YLim = [-1 2];
%       ylabel('Zustand limred'); grid on;
%       xlabel('Iteration lfd Nr');
      
      % Zeile 4
      % Subplots Zweite Spalte -------------------------------------
      subplot(5,18,61:66); hold on;
      wn5_used = false;
      for kk = 1:index_phiz_limred(end)
        if wn5_active_limred(kk,jj) == 1
          wn5_used = true;
          leg2(1) = plot(kk, 1,'om', 'DisplayName','limred aktiv');
        elseif wn5_active_limred(kk,jj) == 0
          leg2(2) = plot(kk, 0,'or', 'DisplayName','limred inaktiv');
        end
      end    
      hold on;
      axe = gca;
      ...
      % Zeile 4
      % Subplots Dritte Spalte -------------------------------------
      subplot(5,18,67:72); hold on;
      wn5_used = false;
      for kk = 1:index_phiz_limred2(end)
        if wn5_active_limred2(kk,jj) == 1
          wn5_used = true;
          plot(kk, 1,'om', 'DisplayName','limred aktiv');
        elseif wn5_active_limred(kk,jj) == 0
          plot(kk, 0,'or', 'DisplayName','limred inaktiv');
      ...
      hold on;
      axe = gca;
      axe.XLim = [index_phiz_limred(1) index_phiz_limred2(end)];
      axe.YLim = [-1 2];
      ylabel('Zustand limred'); grid on;
      xlabel('Iteration lfd Nr'); 
      ...
      % Subplots Dritte Spalte -------------------------------------
      for kkk = 1:RS.NQJ
        subplot(5,18,84+kkk);hold on;
        plot(index_phiz_limred2, Q_save_limred2(1:index_phiz_limred2(end),kkk,jj)/RS.qunitmult_eng_sci(kkk));
      ...
      sgtitle(txt);
      filename_pre = sprintf('EP_Punkt%d_qstart_%d°', jj, q0_diff);
%       filename_pre = sprintf('EP_Punkt%d_qstart_q0rng20°', jj);
%       saveas(f0, fullfile(respath, [filename_pre,'.png'])); 
%       close all;
    end
    
    fprintf('Plot für Einzelpunkte beendet\n'); 
  end
  ...
  if calc_traj
    ...
    % 3T2R-Referenz-Trajektorie
    s_traj_3T2R_ges = zeros(15,1); % wn(:)=0 -> keine Opt.Krit
    ...
    s_traj_limred_qlim_v2       = [0;0];              % qlim quad wn(6) und qlim hyp wn(7)w
    s_traj_limred_cond_v2       = 0;                  % Konditionszahl wn(8)
    s_traj_limred_collision     = [0;0];              % Kollisionsvermeidung wn(9+10)
    s_traj_limred_limredcoord   = [0;0;1;1;0];        % wn(11+12) quard. für qD und qDD, wn(13+14) hyp. für qD und qDD und wn(15) quadr. für qD
...
%     q0_traj = q0 + [0;0;0;0;0;80]*pi/180;
%     q0_traj = q0 + [0;0;0;0;0;100]*pi/180;
%     q0_traj = q0 + [0;0;0;0;0;0]*pi/180;
    ...
    fprintf('Berechnung der Trajektorien Opt.-IK wn13+14 OHNE wn11+12 beendet.\n');
    
    s_traj_limred2 = s_traj_limred;
    s_traj_limred2.wn(11:15) = [0;0;0;0;1];
    [Q_limred2, QD_limred2, QDD_limred2, Phi_limred2, ~, Stats_Traj_limred2] = RS.invkin2_traj(X,XD,XDD,T,q0_traj,s_traj_limred2);
    fprintf('Berechnung der Trajektorien Opt.-IK wn15 OHNE wn11+12 beendet.\n');
    
    s_traj_limred3 = s_traj_limred;
    s_traj_limred3.wn(11:15) = [0;0;1;1;1];
    [Q_limred3, QD_limred3, QDD_limred3, Phi_limred3, ~, Stats_Traj_limred3] = RS.invkin2_traj(X,XD,XDD,T,q0_traj,s_traj_limred3);
    fprintf('Berechnung der Trajektorien Opt.-IK wn11 wn13+14+15 OHNE wn11+12 beendet.\n');
       
    s_traj_limred4 = s_traj_limred;
    s_traj_limred4.wn(11:15) = [1;1;1;1;1];
    [Q_limred4, QD_limred4, QDD_limred4, Phi_limred4, ~, Stats_Traj_limred4] = RS.invkin2_traj(X,XD,XDD,T,q0_traj,s_traj_limred4);
    fprintf('Berechnung der Trajektorien Opt.-IK wn13+14+15 MIT wn11+12 beendet.\n');
    ...
    [X_ist_limred4, XD_ist_limred4, ~] = RS.fkineEE_traj(Q_limred4, QD_limred4, QDD_limred4);
    X_ist_limred_pre4 = X_ist_limred4;
    XD_ist_limred_pre4 = XD_ist_limred4;
    X_ist_limred_int4 = repmat(X_ist_limred4(1,:), length(T), 1) + cumtrapz(T, XD_ist_limred4);
    X_ist_limred4(:,4:6) = normalizeAngle(X_ist_limred4(:,4:6), X_ist_limred_int4(:,4:6));   
    ...
    for i = 1:length(T)
      ...
      phi_test_voll_limred4   = RS.constr2(Q_limred4(i,:)' , RS.x2tr(X(i,:)'), true); 
      ...
      phi_test_red_limred4   = phi_test_voll_limred4(I_IK);
      ...
      if max(abs(phi_test_red_limred4)) > Phirt_tol
        error('IK MIT Limitierung4 stimmt nicht');
      end
    end
    ...
    phiz_traj_diff_limred4       = -X(:,6) + X_ist_limred4(:,6);
    phiz_traj_diff_limred_pre4   = -X(:,6) + X_ist_limred_pre4(:,6);
...
%     plot(T, Stats_Traj_limred.h(1:index_traj(end),8), 'm', 'LineWidth',1);
%     ax_test = gca;
%     ax_test.XLim = [0 T(end)];
%     hold on;
...
%     xlabel('Zeit in s');
    ...
    leg2(1) = plot(T, Stats_Traj_limred.h(1:index_traj(end),9), 'm', 'LineWidth',1, 'DisplayName','h aus IK-Traj');
    ax_test = gca;
    ax_test.XLim = [0 T(end)];
    hinf_used = false;  % Abfrage bzgl. Legendeneintrag
...
%     hdl6 = plot(T, Stats_Traj_limred.h(1:index_traj(end),10), 'm', 'LineWidth',1);
%     ax_test = gca;
%     ax_test.XLim = [0 T(end)];
%     for kk = 1:index_traj(end)
%       if isinf(Stats_Traj_limred.h(kk,10))
%         plot(kk, 0,'*g');
%       end
%     end
%     hold on;
%     ylabel('Optim.krit h(9)'); grid on;
%     xlabel('Zeit in s');
    ...
    ax_test = gca;
    ax_test.XLim = [0 T(end)];
    title('limred 0 0 0 0 1');
    ylabel('phi_z in °'); grid on;
    xlabel('Zeit in s');

%     % Plot von h(7) limred
%     subplot(5,5,8); hold on;
%     plot(T, Stats_Traj_limred2.h(1:index_traj(end),8), 'm', 'LineWidth',1);
%     ax_test = gca;
%     ax_test.XLim = [0 T(end)];
%     hold on;
%     ylabel('Optim.krit h(7)'); grid on;
...
%     plot(T, Stats_Traj_limred2.h(1:index_traj(end),9), 'm', 'LineWidth',1, 'DisplayName','h aus IK-Traj');
%     ax_test = gca;
%     ax_test.XLim = [0 T(end)];
%     hinf_used = false;  % Abfrage bzgl. Legendeneintrag
%     for kk = 1:index_traj(end)
...
%     huneq0_used = false;
    ...
    plot(T, Stats_Traj_limred2.h(1:index_traj(end),10), 'm', 'LineWidth',1);
    ax_test = gca;
    ax_test.XLim = [0 T(end)];
    for kk = 1:index_traj(end)
      if isinf(Stats_Traj_limred2.h(kk,10))
...
%     end
    ...
    ax_test = gca;
    ax_test.XLim = [0 T(end)];
    title('limred 0 0 1 1 1');
    ylabel('phi_z in °'); grid on;
    xlabel('Zeit in s');
    
%     % Plot von h(7) limred
%     subplot(5,5,9); hold on;
%     plot(T, Stats_Traj_limred3.h(1:index_traj(end),8), 'm', 'LineWidth',1);
%     ax_test = gca;
%     ax_test.XLim = [0 T(end)];
%     hold on;
%     ylabel('Optim.krit h(7)'); grid on;
%     xlabel('Zeit in s');
    ...
    plot(T, Stats_Traj_limred3.h(1:index_traj(end),9), 'm', 'LineWidth',1, 'DisplayName','h aus IK-Traj');
    ax_test = gca;
    ax_test.XLim = [0 T(end)];
    hinf_used = false;  % Abfrage bzgl. Legendeneintrag
    for kk = 1:index_traj(end)
    ...
    plot(T, Stats_Traj_limred3.h(1:index_traj(end),10), 'm', 'LineWidth',1);
    ax_test = gca;
    ax_test.XLim = [0 T(end)];
    for kk = 1:index_traj(end)
      if isinf(Stats_Traj_limred3.h(kk,10))
        plot(kk, 0,'*g');
      end
...
% ---------------------------- Subplot fünfte Spalte -------------------
    % Plot von phi_z limred
    subplot(5,5,5); hold on;
    plot(T, phiz_traj_diff_limred4*180/pi, 'm', 'LineWidth',1);
    plot(T, phiz_traj_diff_limred_pre4*180/pi, 'k--', 'LineWidth',1);
    plot(T, xlim_vek(:,1), 'r--', 'LineWidth',1);
    plot(T, xlim_vek(:,2), 'r--', 'LineWidth',1);
    plot(T, xlim_vek_thresh(:,1), 'b--', 'LineWidth',1);
    plot(T, xlim_vek_thresh(:,2), 'b--', 'LineWidth',1);
    ax_test = gca;
    ax_test.XLim = [0 T(end)];
    title('limred  1 1 1 1 1');
    ylabel('phi_z in °'); grid on;
    xlabel('Zeit in s');
    
    % Plot von h(7) limred
    subplot(5,5,10); hold on;
    plot(T, Stats_Traj_limred4.h(1:index_traj(end),8), 'm', 'LineWidth',1);
    ax_test = gca;
    ax_test.XLim = [0 T(end)];
    hold on;
    ylabel('Optim.krit h(7)'); grid on;
    ...
    plot(T, Stats_Traj_limred4.h(1:index_traj(end),9), 'm', 'LineWidth',1, 'DisplayName','h aus IK-Traj');
    ax_test = gca;
    ax_test.XLim = [0 T(end)];
    hinf_used = false;  % Abfrage bzgl. Legendeneintrag
    for kk = 1:index_traj(end)
      if isinf(Stats_Traj_limred4.h(kk,9))
        hinf_used = true;
        plot(kk, 0,'*g', 'DisplayName','h = inf');
      end
    end
    ...
    plot(T, Stats_Traj_limred4.h(1:index_traj(end),10), 'm', 'LineWidth',1);
    ax_test = gca;
    ax_test.XLim = [0 T(end)];
    for kk = 1:index_traj(end)
      if isinf(Stats_Traj_limred4.h(kk,10))
        plot(kk, 0,'*g');
      end
%       if Stats_Traj_limred3.h(kk,9) ~= 0 && ~isinf(Stats_Traj_limred3.h(kk,9))
%         plot(kk, 0,'*r');
%       end
    ...
    plot(T, Stats_Traj_limred4.phi_zD(1:index_traj(end),1)*180/pi, 'm', 'LineWidth',1);
    ax_test = gca;
    ax_test.XLim = [0 T(end)];
    hold on;
    ylabel('Geschwindigkeit von phi_z in °/s'); grid on;
    ...
    fprintf('Plot für Trajektorie beendet\n'); 
...
%% Trajektorie aus Gelenkraum visualisieren
  for qplots = 5:5
    ...
    for k = 1:RS.NQJ
      ...
      elseif qplots == 2, sgtitle('Opt. Traj 0 0 1 1 0');
      elseif qplots == 3, sgtitle('Opt. Traj 0 0 0 0 1');
      elseif qplots == 4, sgtitle('Opt. Traj 0 0 1 1 1');
      elseif qplots == 5, sgtitle('Opt. Traj 1 1 1 1 1');
      end
    end
    linkxaxes
