  ...
  methods
    ...
    function [Q,QD,QDD,PHI,JointPos_all,Stats] = invkin2_traj(R, X, XD, XDD, T, q0, s_in)
      ...
      s = struct( ...
         ...
         'enforce_xlim', true, ... % Einhaltung der Winkelgrenzen für den Endeffektor (bei Nullraumbewegung)
         'enforce_xDlim', true, ... % Für Endeffektor-Geschwindigkeits-Grenzen
         'xlim', R.xlim, ...
         'xDlim', R.xDlim, ...
         'xDDlim', R.xDDlim, ...
    ...
    function update_EE_FG(R, ~, I_EE_Task)
      ...
      elseif all(I_EE_Task == [1 1 1 0 0 0]) % 3T3R
        if all(R.I_EE == [1 1 1 0 0 1]) || all(R.I_EE == [1 1 1 0 0 0])
          ...
          % Allgemeiner Fall. Betrachte ohne Redundanz
          R.I_constr_red = 1:6;
          R.I_constr_t_red = R.I_constr_t;
          R.I_constr_r_red = R.I_constr_r;
        end
      elseif all(I_EE_Task == [1 1 0 0 0 1]) % 2T1R
        R.I_constr_red = [1 2];
        R.I_constr_t_red = [1 2];
        R.I_constr_r_red = 6;
      elseif all(I_EE_Task == [1 1 0 0 0 0]) % 2T0R
        R.I_constr_red = [1 2];
        R.I_constr_t_red = [1 2];
        R.I_constr_r_red = [];
      else
