...
function [Phi_q_red, Phi_q] = constr4gradD_q(Rob, q, qD)

%% Initialisierung
assert(isreal(q) && all(size(q) == [Rob.NJ 1]), ...
  'ParRob/constr4gradD_q: q muss %dx1 sein', Rob.NJ);
assert(isreal(qD) && all(size(qD) == [Rob.NJ 1]), ...
  'ParRob/constr4gradD_q: qD muss %dx1 sein', Rob.NJ);
%% Aufruf der Unterfunktionen
% Die Unterfunktionen sind nach ZB-Art sortiert, in der Ausgabevariablen
% ist die Sortierung nach Beingruppen (ZB Bein 1, ZB Bein 2, ...)
[Phi_tq_red,Phi_tq]=Rob.constr1gradD_tq(q, qD); % Identisch zu Methode 1
[Phi_rq_red,Phi_rq]=Rob.constr4gradD_rq(q, qD); % Methode 4
