...
for k = 1:nt
  ...
  if ~taskred_rot || ... % beliebige PKM (3T0R, 3T1R, 3T3R) ohne Aufg.Red.
    ...
    qD_k_T = J_x_inv * xD_k(Rob_I_EE); % ohne Redundanz nur Aufgaben-Geschw.; eindeutig.
  else
  ...
  elseif ~s.ik_solution_min_norm && taskred_rot
    % Benutze nicht die Jacobi-Matrix-Zeitableitung. TODO: Genaue
    % Begründung fehlt noch. Führt aber unten zu keinem Fehler.
    qDD_k_T = J_x_inv * xDD_k(Rob_I_EE) + ...
      -Phi_q_voll\(Phi_qD_voll*qD_k+Phi_xD_voll*xD_k(Rob_I_EE));
  else
  ...
  if debug % Erneuter Test
    ...
    if any(abs(PhiDD_test3) > max(1e-6, max(abs([qD_k;qDD_k_T]))/1e9)) % bei hohen Werten ist die Abweichung größer; feine IK-Toleranz notwendig.
      error(['Schritt %d: Beschleunigung qDD_k_T erfuellt die kinematischen Bedingungen ', ...
        'nicht. Max. Fehler %1.2e'], k, max(abs(PhiDD_test3)));
    end
  end
  if nsoptim || redundant
  ...
  if nsoptim || ... % Nullraumbewegung: Zwei Koordinatenräume dafür möglich (s.u.)
    ...
    if ~isempty(s.nullspace_maxvel_interp) && k < nt
      ...
      if vmax_rel == 1 % Halte die relative Geschw. bzgl. der Grenzen fest
        ...
        vmax_rel_lastvel = max(vmax_rel_lastvel, 1.5*max(max(repmat(qD_k-qD_k_T,1,2)./qDlim)));
    ...
    if ~isempty(s.xlim6_interp)
      ...
      % Wähle mindestens eine Aktivierungsschwelle bei 80% der Grenzen
      % (also 10% der Spannweite auf jeder Seite der Grenze). Hyperbolisch
      % wird es ja erst bei der Hälfte von da bis zur Grenze.
      delta_phiz = max(delta_phiz, (xlim(6,2)-xlim(6,1))*0.1);
      xlim_thr_h10 = xlim;
