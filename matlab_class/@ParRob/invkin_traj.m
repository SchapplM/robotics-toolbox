    ...
    if (condJ >= thresh_ns_qa || sum(Rob.I_qa) ~= sum(Rob.I_EE) || debug) && ...
      ...
      if debug && condJ < thresh_ns_qa && sum(Rob.I_qa) == sum(Rob.I_EE) && ...
        ...
        if ~any(isnan(test_qDD_N_ratio)) && ... % bei exakt h=0 kommt sonst Fehler
          abs(diff(minmax2(test_qDD_N_ratio_rel'))) > 5e-2 % 5% genau (für kleine Zahlenwerte)
        ...
        if ~any(isnan([test_qDD_N_ratio;test_qaDD_N_ratio])) && ... % bei exakt h=0 kommt sonst Fehler
          (abs(diff(minmax2(test_qDD_N_ratio_rel'))) > (1+condJ/1e3)*1e-2 || ... % Teste auf 1% genau. Lasse mehr Fehler zu, wenn schlecht konditioniert.
           abs(diff(minmax2(test_qaDD_N_ratio_rel'))) > (1+condJ/1e3)*1e-2 || ...
           abs(diff(minmax2([test_qaDD_N_ratio_rel',test_qDD_N_ratio_rel']))) > (1+condJ/1e3)*1e-2)
